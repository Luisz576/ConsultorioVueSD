<script setup lang="ts">
  ...
  async function _new(){
    if(!creating){
      creating = true
      try{
        if(person.value.name.length > 2 && person.value.cpf.length == 11){
          const res = await http.post('http://127.0.0.1:5000/api/patient', {
...
<template>
  <div v-show="props.isVisible" class="top-0 left-0 fixed w-full h-full">
    ...
    <div class="px-10 pt-8 pb-6 fixed bg-zinc-300 z-20 w-96" style="left: calc(50% - 12rem); top: calc(20%);">
      <h3 class="text-950 font-bold text-lg underline">Novo Paciente</h3>
      <div class="mt-2">
        <div class="mb-2">
